...
def answer(entrances, exits, path):
    total_num_of_bunnies = 0
    length = range(0, len(path[0]))

    # Send bunnies from first room to intermediate rooms
    bunny_attendance = [[0, 0] for _ in length]
    for k in length:
        bunny_attendance[k][0] = sum(path[k])

    # calculate bunnies and capacity of each room
    # for i in entrances:
    #     total_num_of_bunnies += sum(path[i])
    #     for j in length:
    #         bunny_attendance[j][1] += path[i][j]
    #         if bunny_attendance[j][1] > bunny_attendance[j][0]:
    #             bunny_attendance[j][1] = bunny_attendance[j][0]

    for x in length:
        if x not in entrances:
            for y in length:
                if y not in entrances and path[x][y] != 0:
                    bunny_attendance[y][1] += path[x][y]
                    if bunny_attendance[y][1] > bunny_attendance[y][0]:
                        if bunny_attendance[y][0] != 0:
                            bunny_attendance[y][1] = bunny_attendance[y][0]

    return bunny_attendance
